...
parser.add_argument('--dataset', type=str, default="IndianPines", choices=dataset_names,
                    help="Dataset to use.")
parser.add_argument('--model', type=str, default="SSPN",
                    help="SSPN")
...
if palette is None:
    ...
    labels = loadmat(os.path.join('./Datasets/Chikusei_MATLAB', 'HyperspecVNIR_Chikusei_20140729_Ground_Truth.mat'))['GT']
    labels_color = labels[0][0]['classcolor']
    palette = {0: (0, 0, 0)}
    for i in range(len(labels_color)):
        palette[i + 1] = labels_color[i]
...
# 如果为hybridsn，则先对图片进行PCA降维
if MODEL == 'hybridsn_ps25' or MODEL == 'ssfttnet':
    a_shape = img.shape
    b = img.reshape(-1, img.shape[2])
    c = PCA(n_components=30).fit_transform(b)
    d = np.array(a_shape)
    d[2] = 30
    img = c.reshape(d)
...
for run in range(N_RUNS):
    ...
    if MODEL == 'SVM_grid':
        ...
        # Neural network
        model, optimizer, loss, hyperparams = get_model(MODEL, ps=[3, 5], d_h=[2, 4], **hyperparams)
        # Split train set in train/val
        train_gt, val_gt = sample_gt(train_gt, 0.75, mode='random')
    ...
    print("test_gt is :", test_gt)
    # print("outputs:", outputs)
    run_results = metrics(prediction, test_gt, ignored_labels=hyperparams['ignored_labels'], n_classes=N_CLASSES)

    # 如果想得到只有标注样本的预测图，就用以下代码
